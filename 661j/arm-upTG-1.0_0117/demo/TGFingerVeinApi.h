...
extern "C" {
	...
	Output：
		unsigned char *feature：从加密的指静脉图片中提取到的指静脉特征数据缓存区指针（缓存区 1936 Bytes）
	Return：
	...
	**************************************************************************************/
	...
	Input：
		unsigned char *feature1：待融合的特征数据1缓存区指针（缓存区 1936 Bytes）
		unsigned char *feature2：待融合的特征数据2缓存区指针（缓存区 1936 Bytes）
		unsigned char *feature3：待融合的特征数据3缓存区指针（缓存区 1936 Bytes）
	Output：
		unsigned char *tmpl：三个特征融合成的模板数据缓存区指针（缓存区5808 Bytes）
	Return：
	...
	**************************************************************************************/
	...
	Input：
		unsigned char *feature：待比对的特征数据缓存区指针（缓存区 1936 Bytes）
		unsigned char *tmpl：模板数据缓存区指针（缓存区 5808 Bytes）
	Output：
		unsigned char *updateTmpl：保留参数（缓存区 5808 Bytes）
	Return：
		0：特征比对成功
		else：特征比对失败
	Others：
	...
	**************************************************************************************/
	...
	Input：
		unsigned char *feature：待比对的特征数据缓存区指针（缓存区 1936 Bytes）
		unsigned char *tmplStart：地址连续的的多个模板数据缓存区指针（缓存区 tmplNum*5808 Bytes）
		int tmplNum：地址连续的多个模板的个数
	Output：
		int *matchIndex：在地址连续的多个模板数据中找到与之相匹配的模板数据其位置的变量指针
		unsigned char *updateTmpl：比对成功后，与之相匹配的模板的更新数据（缓存区 5808 Bytes）
	Return：
